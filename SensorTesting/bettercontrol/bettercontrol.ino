...
float computePID(float error) {
  ...
  filtered_derivative = 0.2 * raw_derivative + 0.8 * filtered_derivative;
  float derivative = filtered_derivative;

  // --- Predict output BEFORE integrating ---
  float u_unsat = Kp * error + Ki * integral + Kd * derivative;

  // --- Anti-windup: only integrate if not saturated ---
  if (abs(u_unsat) < OUTPUT_LIMIT) {
    integral += error * dt;
...
void dive() {
  ...
  switch (wpstate) {
    ...
    case TRAVELING:
      uV = computePID(depth_error);

      if (atWaypoint) {
        wpstate = HOVERING;
        stateStartTime = currentTime;

        // reset PID to prevent carryover
        integral = 0;
        prev_error = 0;
        lastPIDTime = millis();
      }
